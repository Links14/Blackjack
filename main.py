...
def reshuffle(d, p_hands, d_hand):
    ...
    if len(d) < 2* (len(p_hands) + 1): # 2 cards per player + 1 for the dealer hand
        print("Shuffling remainin cards.")
        d = NEW_DECK[:]
        random.shuffle(d)
        for i in p_hands:
            for j in i:
        ...
        while True:
            ...
            for num in range(len(player_hands)):
                ...
                if dealer_hand[0][1] == "A":
                    ...
                    f"\n\033[36mMin Value: 2\033[0m, " +
                    f"\n\033[36mMax Value: 21\033[0m\n------------------------------")
                else:
                    tmp = calc_hand([dealer_hand[0]])[0]
                    print(f"Dealer has {deck[dealer_hand[0]]} and a hidden card. " +
                        f"\n\033[36mMin hand value: {tmp + 1} \033[0m" +
                ...
                while player_turn:
                    try:                                        
                        ...
                        if user_input == 1:
                            ...
                            hit_res = hit(player_hands[num], playing_deck)
            ...
            while True:
                ...
                if not dealer_stand:
                    ...
                    time.sleep(0.5)
                    hit_res = hit(dealer_hand, playing_deck)
                    time.sleep(0.5)
                    # Show dealer's draw
                    print(f"Dealer drew a {deck[player_hands[num][-1]]}. New\033[36m value: ", end="")
                    d_calc = calc_hand(dealer_hand)
